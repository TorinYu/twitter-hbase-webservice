...
def main():
 ...
 #   cursor.execute ("CREATE DATABASE twitter")
   ...
   """)
    ...
    mybucket = conn.get_bucket('uidcount')
    set = mybucket.list()
    for i in set:
            s=i.name
            s = "http://s3.amazonaws.com/uidcount/" + s
            ...
            for tweetJasons in tweets:
                    ...
                    userid = tweetJasons.split('\t')[0]
 		    
                    count = tweetJasons.split('\t')[1]
 ...
 #                   print userid + ' '+ tid+ ' '+ content +' ' + create_at + '\n'   
                    ...
                    cursor.execute ("""INSERT INTO tweetsq3   (userid, count) VALUES  (%s, %s)""", (userid, count))
    #Commit the changes.
    connSQL.commit()
